...
//Algo: Reverse regularly, then reverse with the chunks
struct Node {
    int val; 
    int k;
    struct Node* next;
...
NodeAddress reverseLLWithNextK(NodeAddress head){
    ...
    //Algo: reverse next, then just reassign nextk for each node while reversing. k is stored on the node so it's trivial.
    ...
    while(current) {
        temp = current -> next;
        current-> next = prev;
        current -> nextk = skip_chunk(current->k, current);
        prev = current;
        current = temp;
    }
    return prev;
